...
checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=0, save_best_only=False, mode='auto')
...
#print "-----------------------------"
# no output if verbose=False
history = model.fit(X_train, Y_train, nb_epoch=30, batch_size=16, shuffle=True, verbose=False, callbacks=[checkpoint])
...
print 'y[0]= ', y[0], ',  Y_test[0]= ', Y_test[0]
print 'y= ', y
...
#quit()
# 
nb_errors= np.abs(diff).sum() / 2
frac_errors= nb_errors / len(y1)
print "nb errors: ", nb_errors
print "frac errors: ", frac_errors
quit()
#------------------------------------------------------
